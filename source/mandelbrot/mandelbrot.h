...
public:
  ///////////////////////////////////////////////////////////////////////////////////////////////////
  mandelbrot()
      : m_max_channel_value{std::numeric_limits<T>::max()}
      , m_max_iterations{m_max_channel_value}
  {
    ;
  }
  ...
  ///////////////////////////////////////////////////////////////////////////////////////////////////
  auto pixel_at_iteration(const int32_t iteration) -> std::tuple<T, T, T, T>
  {
...
///////////////////////////////////////////////////////////////////////////////////////////////////
// Template to create a 1d vector with interlaced rgba pixel values, for a Mandelbrot set
template <typename T>
auto create_vec_1d_filled_with_mandelbrot_pixels(const double   x_max,
                                                 const double   x_min,
                                                 const double   y_max,
                                                 const double   y_min,
                                                 const uint32_t width,
                                                 const uint32_t height) -> jaimerios::vec_1d<T>
{
  // This template function will fill up a 1d vec with the pixel data that you need for a
  // Mandelbrot image
  const auto channels = 4u;

  auto image  = jaimerios::mandelbrot<T>{};
  auto pixels = std::vector<T>(width * height * channels, 0);
  ...
  for (auto y = 0; y < height; ++y)
  {
    ...
    {
      const auto scaled_x = scale_coordinate<double>(x, x_min, x_max, width);
      const auto scaled_y = scale_coordinate<double>(y, y_min, y_max, height);
